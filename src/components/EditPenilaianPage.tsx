...
const EditPenilaianPage: React.FC = () => {
  const { id_penilaian } = useParams<{ id_penilaian: string }>();
  const navigate = useNavigate();
  const location = useLocation();
  const state = location.state as LocationState;
  const student = state?.student;

  const [studentDetails, setStudentDetails] = useState<any>(student || null);
  ...
  useEffect(() => {
    ...
    const fetchInitialData = async () => {
      try {
        // Fetch specific penilaian
        const resPenilaian = await api.get(`/penilaian/${id_penilaian}`);
        if (resPenilaian.data) {
          const fetchedData = resPenilaian.data;
          setCatatan(fetchedData.catatan_umum || '');
          if (!studentDetails) {
            setStudentDetails({
              id_siswa: fetchedData.evaluatee.id_siswa,
              nama: fetchedData.evaluatee.nama_siswa,
              kelas: '',
              foto: fetchedData.evaluatee.jenis_kelamin === 'laki_laki' ? '/images/default-male.jpg' : '/images/default-female.jpg'
            });
          }
        }
        ...
        if (resCat.data && resCat.data.length > 0) {
          mappedCategories = resCat.data;
          setCategories(resCat.data);
        } else {
          setCategories([]);
        }

        // Merge initial ratings with fetched logic
        const initialRatings: Record<string, number> = {};
        mappedCategories.forEach((cat: any) => initialRatings[cat.id_kategori] = 0);

        if (typeof resPenilaian !== 'undefined' && resPenilaian.data && resPenilaian.data.detail) {
          resPenilaian.data.detail.forEach((d: any) => {
            if (d.kategori?.id_kategori) {
              initialRatings[d.kategori.id_kategori] = d.skor;
            }
          });
        }
        ...
        console.error('Failed to fetch initial edit data:', error);
      }
    };

    fetchInitialData();
  }, [navigate, id_penilaian, studentDetails]);
  ...
  const handleSubmit = async (isSaveAndContinue: boolean) => {
    try {
      ...
      await api.patch(`/penilaian/${id_penilaian}`, {
        id_guru: guruId,
        id_siswa: studentDetails?.id_siswa,
        periode: 'Minggu ini',
          ...
          <div className="form-card" style={{ maxWidth: '800px', margin: '0 auto' }}>
            <div className="form-header student-profile-header">
              <div className="student-photo large">
                {studentDetails?.foto ? (
                  <img src={studentDetails.foto} alt={studentDetails?.nama} style={{ borderRadius: '50%', width: '80px', height: '80px', objectFit: 'cover' }} />
                ) : (
                  <div className="student-avatar-placeholder large">{getInitials(studentDetails?.nama || 'S')}</div>
                )}
              </div>
              <div className="student-info-main">
                <h2 style={{ marginBottom: '0.25rem' }}>{studentDetails?.nama || 'Nama Siswa'}</h2>
                <span className="student-class-badge">{studentDetails?.kelas || 'Kelas'}</span>
              </div>
            </div>
