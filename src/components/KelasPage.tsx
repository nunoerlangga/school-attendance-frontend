...
const KelasPage: React.FC = () => {
  ...
  const fetchKelas = async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await api.get(`/kelas?is_aktif=${showActive}`);
      // Sort: Active first, then by name
      const sorted = res.data.sort((a: Kelas, b: Kelas) => {
  ...
  // Filter logic
  const filteredList = kelasList.filter((item) => {
    const nama = item.nama_kelas || '';
    const tingkat = item.tingkat || '';
    const search = searchTerm.toLowerCase();
    return nama.toLowerCase().includes(search) || tingkat.toLowerCase().includes(search);
  });
  ...
  const handleSoftDelete = async (id: string, is_aktif: boolean) => {
    const act = is_aktif ? 'menonaktifkan' : 'mengaktifkan';
    const confirmDelete = window.confirm(`Apakah kamu yakin ingin ${act} kelas ini?`);
    if (!confirmDelete) return;
    try {
      setActionLoading(`soft-${id}`);
      await api.patch(`/kelas/soft-delete/${id}`);
      await fetchKelas();
    } catch (err: any) {
      alert(err.response?.data?.message || `Gagal ${act} kelas.`);
    } finally {
    ...
    <div className="data-page">
      <div className="card-header">
        <div className="header-info">
          <h2>Data Kelas</h2>
          <p>Kelola daftar kelas dan rombongan belajar.</p>
        </div>
        <div className="header-actions" style={{ display: 'flex', gap: '1rem', alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <button
            ...
            onClick={() => navigate('/admin/create-kelas')}
          >
      ...
      <div className="table-container">
        {loading ? (
          ...
          <div className="empty-state" style={{ padding: '3rem', textAlign: 'center', color: 'var(--text-muted)' }}>
            {searchTerm ? `Tidak ditemukan kelas dengan kata kunci "${searchTerm}"` : 'Belum ada data kelas.'}
          </div>
        ) : (
          <>
              ...
              <tbody>
                {paginatedList.map((item, index) => (
                  <tr key={item.id_kelas}>
                    ...
                    <td className="font-bold">{item.nama_kelas}</td>
                    <td>{item.tingkat}</td>
                    ...
                    <td>
                      <div className="action-buttons">
                        <button
                          ...
                          title="Edit"
                          onClick={() => navigate(`/admin/edit-kelas/${item.id_kelas}`)}
                        ...
                        <button
                          ...
                          onClick={() => handleSoftDelete(item.id_kelas, item.is_aktif)}
                          disabled={actionLoading !== null}
                          style={!item.is_aktif ? { color: 'var(--success)' } : {}}
                        >
                          {actionLoading === `soft-${item.id_kelas}` ? (
