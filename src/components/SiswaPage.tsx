...
const SiswaPage: React.FC = () => {
  ...
  const fetchSiswas = async () => {
    try {
      setLoading(true);
      const response = await api.get(`/siswa?is_aktif=${showActive}`);
  ...
  const handleToggleActive = async (id: string, name: string, isAktif: boolean) => {
    ...
    if (window.confirm(`Apakah Anda yakin ingin ${action} siswa ${name}?`)) {
      try {
        await api.delete(`/siswa/${id}`); // Toggles backend status
        fetchSiswas();
      } catch (error) {
        console.error(`Error ${action} student:`, error);
        alert(`Gagal ${action} siswa`);
      }
    }
  };
  const filteredSiswas = siswas.filter(siswa =>
    siswa.nama_siswa.toLowerCase().includes(searchTerm.toLowerCase()) ||
    siswa.user.username.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <div className="data-page">
      <div className="card-header">
        <div className="header-info">
          <h2>Data Siswa</h2>
          <p>Kelola data seluruh siswa di sekolah.</p>
        </div>
        <div className="header-actions" style={{ display: 'flex', gap: '1rem', alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <button
            ...
            onClick={() => navigate('/admin/create-siswa')}
          >
              ...
              {filteredSiswas.length > 0 ? (
                filteredSiswas.map((siswa) => (
                  <tr key={siswa.id_siswa}>
                    <td className="font-mono">{siswa.user.username}</td>
                    <td className="font-bold">{siswa.nama_siswa}</td>
                    <td>{siswa.jenis_kelamin === 'laki_laki' ? 'L' : 'P'}</td>
                    <td>
                      {siswa.kelas && siswa.kelas.length > 0
                        ? siswa.kelas[0].kelas.nama_kelas
                        : '-'}
                    </td>
                    <td>
                      <span className={`status-badge ${siswa.user.is_aktif ? 'active' : 'inactive'}`}>
                        {siswa.user.is_aktif ? 'Aktif' : 'Nonaktif'}
                      </span>
                    </td>
                    <td>
                      <div className="action-buttons">
                        <button
                          ...
                          onClick={() => navigate(`/admin/edit-siswa/${siswa.id_siswa}`)}
                          title="Edit"
                        >
                        ...
                        <button
                          ...
                          onClick={() => handleToggleActive(siswa.id_siswa, siswa.nama_siswa, siswa.user.is_aktif)}
                          title={siswa.user.is_aktif ? "Nonaktifkan" : "Aktifkan"}
                        >
                          {siswa.user.is_aktif ? (
